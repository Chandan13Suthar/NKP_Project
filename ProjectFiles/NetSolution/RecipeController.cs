...
public class RecipeController : BaseNetLogic
{
        ...
        try
        {
            ...
            if (store != null && editModel != null)
            {
                if (RecipeExistsInStore(store, schema, name))
                {
                    ...
                    // Recipe Saved Activity Logging into Audit Database
                    AuditTrailLogging RecipeSavedLog = new AuditTrailLogging();
                    RecipeSavedLog.LogIntoAudit("Recipe saved", name, Session.User.BrowseName, "RecipeApplicationEvent");
                ...
                else
                {
                    ...
                    //-----------Customized Logic Start-----------------
                    // Recipe Created Activity Logging into Audit Database
                    AuditTrailLogging RecipeCreatedLog = new AuditTrailLogging();
                    RecipeCreatedLog.LogIntoAudit("Recipe created", name, Session.User.BrowseName, "RecipeApplicationEvent");
                    //-----------Customized Logic End-------------------
        ...
        try
        {
            ...
            if (editModel != null)
            {
               schema.Copy(schema.TargetNode, editModel.NodeId, ErrorPolicy);
                ...
                //-----------Customized Logic Start-----------------
                // Recipe Loaded Activity Logging into Audit Database
                AuditTrailLogging RecipeLoadedLog = new AuditTrailLogging();
                string RecName1 = GetRecipeName();
                RecipeLoadedLog.LogIntoAudit("Recipe loaded", RecName1, Session.User.BrowseName, "RecipeApplicationEvent");
                //-----------Customized Logic End-------------------
    ...
    public void ApplyFromEditModel(CopyErrorPolicy ErrorPolicy)
    {
        ...
        try
        {
            ...
            if (editModel != null)
            {
                ...
                //-----------Customized Logic Start-----------------
                // Recipe Applied Activity Logging into Audit Database
                AuditTrailLogging RecipeAppliedLog1 = new AuditTrailLogging();
                string RecName2 = GetRecipeName();
                RecipeAppliedLog1.LogIntoAudit("Recipe applied", RecName2, Session.User.BrowseName, "RecipeApplicationEvent");
                //-----------Customized Logic End-------------------
    ...
    public void ApplyFromDB(string RecipeName, CopyErrorPolicy ErrorPolicy)
    {
        ...
        try
        {
            ...
            //-----------Customized Logic Start-----------------
            // Recipe Applied Activity Logging into Audit Database
            AuditTrailLogging RecipeAppliedLog2 = new AuditTrailLogging();
            string RecName3 = GetRecipeName();
            RecipeAppliedLog2.LogIntoAudit("Recipe applied", RecName3, Session.User.BrowseName, "RecipeApplicationEvent");
            //-----------Customized Logic End-------------------
    ...
    public void Export()
    {
        ...
        try
        {
            ...
            //-----------Customized Logic Start-----------------
            // Recipe Exported Activity Logging into Audit Database
            AuditTrailLogging RecipeExportedLog = new AuditTrailLogging();
            RecipeExportedLog.LogIntoAudit("Recipe exported", csvPath, Session.User.BrowseName, "RecipeApplicationEvent");
            //-----------Customized Logic End-------------------
    ...
    public void Import()
    {
        ...
        try
        {
            using (var csvReader = new CSVFileReader(csvPath) { FieldDelimiter = separator, IgnoreMalformedLines = true, WrapFields = wrapFields })
            {
                ...
                //-----------Customized Logic Start-----------------
                // Recipe Imported Activity Logging into Audit Database
                AuditTrailLogging RecipeImportedLog = new AuditTrailLogging();
                RecipeImportedLog.LogIntoAudit("Recipe imported", csvPath, Session.User.BrowseName, "RecipeApplicationEvent");
                //-----------Customized Logic End-------------------
